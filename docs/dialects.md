...
```
id = "3F2504E0-4F89-11D3-9A0C-0305E82C3301" AND edited > "2 days"
```

```sql
-- postgres
(doc.id = $1::uuid AND EXTRACT(EPOCH FROM (NOW() - rev.created_at)) > $2)
-- duckdb
...
query := "SELECT doc.id, doc.name FROM document doc"
if res.SQL != "" {
    query += " WHERE " + res.SQL
}
...
```go
if slices.Contains(res.Fields, "team") {
    query += " JOIN grp ON grp.id = doc.grp_id"
}
```
...
makes `edited > "2 days"` mean "older than two days":
